...
def index(request):
    ...
    return render(request, "Structure_Design/index.html", {'Project': project, 'Coworking': coworking})

def detail(request, id:int, title:str):
    project = get_object_or_404(STRProject.objects.prefetch_related('images'),id=id)
    related_projects = STRProject.objects.filter(category=project.category).exclude(id=project.id)[:4]
